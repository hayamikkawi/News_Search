...
class MySQLDatabase:
    ...
    def save_article(self, article: ArticleRecord) -> bool:
        """
        Save article record (with full text content)

        Uses "check-then-insert" strategy to avoid AUTO_INCREMENT gaps caused by
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                cursor.execute(
                    ...
                    WHERE url = %s AND feed_url = %s
                    """,
                    (article.url, article.feed_url),
                )
                ...
                if result:
                    ...
                    conn.commit()
                    logger.debug(f"Article already exists, updated: {article.url}")
                    return True

                # Step 2: Article doesn't exist, insert new record
                cursor.execute(
                    """
                ...
                conn.commit()
                logger.debug(f"New article saved: {article.url}")
                return True
            except Error as e:
                conn.rollback()
    ...
    def save_article_metadata_only(self, article: ArticleRecord) -> Optional[int]:
        ...
        with self.get_connection() as conn:
            ...
            try:
                ...
                # Step 1: Check if article already exists (avoid AUTO_INCREMENT waste)
                cursor.execute(
                    """
                    SELECT id FROM articles
                    WHERE url = %s AND feed_url = %s
                ...
                if result:
                    # Article exists, optionally update metadata
                    doc_id = result[0]
                    cursor.execute(
                        """
                        UPDATE articles SET
                    ...
                    conn.commit()
                    logger.info(f"Article already exists, updated metadata for doc_id: {doc_id}")
                    return doc_id
                ...
                # Get the newly inserted doc_id
                doc_id = cursor.lastrowid
                conn.commit()
                logger.info(f"New article metadata saved with doc_id: {doc_id}")
                return doc_id
            except Error as e:
                conn.rollback()

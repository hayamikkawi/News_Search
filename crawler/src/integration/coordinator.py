    ...
    def process_articles_batch(self, articles: List[ArticleRecord]) -> List[ProcessResult]:
        ...
        try:
            if hasattr(self.indexer, "flush"):
                # Use append mode by default for incremental updates
                ...
                if flush_mode == "append_only":
                    logger.warning(
                        "Using 'append_only' mode which may create duplicate IDs. "
                        "Ensure you run periodic dedup_file() or your Indexer can handle duplicates!"
                    )

                self.indexer.flush(mode=flush_mode)
                logger.info(f"Flushed documents to indexer output file (mode: {flush_mode})")
        except Exception as e:
